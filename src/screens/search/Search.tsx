...
import { View, Text, TouchableOpacity, Keyboard, TouchableWithoutFeedback, FlatList } from 'react-native';
...
const Search = React.memo(({ navigation }: any) => {
      ...
      <View style={styles.View}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => {
              return (
                <TouchableOpacity style={styles.fullW} onPress={() => handleTileClick(item)} activeOpacity={0.6} key={item.list_id}>
                  <SearchLists item={item} />
                </TouchableOpacity>
              );
            }}
            ListEmptyComponent={
              searchTerm.length > 0 ? <Text style={{ ...styles.emptyListText, marginTop: 20 }}>{`No lists found matching this name :(`}</Text> : null
            }
          />
        )}
      </View>
