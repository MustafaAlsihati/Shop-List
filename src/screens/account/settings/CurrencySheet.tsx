...
const CurrencySheet = React.memo(({ refRBSheet, userSettings, UpdateSettings, onClose, handleInputChange }: any) => {
  ...
  return (
    <>
      <BottomSheet refRBSheet={refRBSheet} height={450} closeOnDragDown={false} dragFromTopOnly={true}>
        <Input
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <TouchableOpacity key={item.code} onPress={() => handleInputChange(item, 'currency')} activeOpacity={0.6}>
              <List.Item
                key={item.code}
                title={item.code}
                ...
                right={() => <Text style={{ ...styles.centerVH, ...styles.settingText }}>{item.symbol}</Text>}
                style={[
                  {
                    paddingVertical: 0,
                  },
        ...
        <View style={[styles.BSInputFieldContainer, { marginBottom: insets.bottom }]}>
          <Button title="SAVE" buttonStyle={styles.btn} titleStyle={styles.btnTitle} onPress={handleSubmit} />
        </View>
      </BottomSheet>
    </>
  );
