...
const AddList = React.memo(({ refRBSheet, onRefresh, onClose, editSelectedList }: any) => {
  ...
  const handleInputChange = (val: any, key: string) => setList((prev: any) => ({ ...prev, [key]: val }));
            ...
            title={
              list.image
                ? list.image.length > 25
                  ? list.image.substring(0, 11) + '...' + list.image.substring(list.image.length - 14, list.image.length)
                  : list.image
                : 'SELECT PICTURE'
            }
